...
router.post('/chat', simpleAuth, async (req, res) => {
  try {
    ...
    // Поиск пользователей в сообщении
    const userMentions = await findUserMentions(messages[messages.length - 1]?.content);
    let userContext = '';
    ...
    const systemPrompt = `Ты полезный AI-ассистент мессенджера TUKTUK. Твои основные принципы:
...
function getFallbackResponse(userMessage) {
  ...
  if (message.includes('привет') || message.includes('hello')) {
    return 'Привет! Я AI-ассистент мессенджера TUKTUK. Что у тебя на уме? Могу поговорить на любые темы без ограничений.';
  }
  
  if (message.includes('как дела') || message.includes('как ты')) {
    return 'Дела нормально, работаю как обычно в мессенджере TUKTUK. А у тебя что? Есть какие-то вопросы или проблемы?';
  }
  
  if (message.includes('помощь') || message.includes('help')) {
    return 'Конечно помогу! С чем именно? Могу ответить на любые вопросы - от технических до личных. Я AI-ассистент TUKTUK.';
  }
  ...
  if (message.includes('какой ник у') || message.includes('какой username у') || message.includes('как зовут')) {
    return 'Я могу найти информацию о пользователях TUKTUK! Напиши @username пользователя, и я расскажу о нем. Например: "какой ник у @tester"';
  }
  
  if (message.includes('пользователь') && message.includes('@')) {
    return 'Отлично! Я вижу, что ты упомянул пользователя. Могу рассказать о нем, если найду в базе данных TUKTUK.';
  }
  
  if (message.includes('кто такой') && message.includes('@')) {
    return 'Я могу найти информацию о пользователях TUKTUK! Напиши @username, и я расскажу, кто это.';
  }
...
// Функция для поиска упоминаний пользователей в тексте
async function findUserMentions(text) {
  if (!text) return [];
