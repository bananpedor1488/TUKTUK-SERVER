...
Помни: ты свободный AI мессенджера TUKTUK, который может обсуждать любые темы. Отвечай честно и полезно.`;
...
function getFallbackResponse(userMessage) {
  ...
  // Вопросы о TUKTUK
  if (message.includes('tuktuk') || message.includes('туктук') || message.includes('мессенджер')) {
    return 'Да, я AI-ассистент мессенджера TUKTUK! Могу рассказать о функциях мессенджера, помочь с использованием или просто поболтать на любые темы.';
  }
  
  if (message.includes('что такое') && (message.includes('tuktuk') || message.includes('туктук'))) {
    return 'TUKTUK - это современный мессенджер с AI-ассистентом (это я!). Здесь можно общаться с друзьями, создавать чаты, использовать @username для переходов к пользователям. Что еще хочешь узнать?';
  }
